...
-   Reproducibility is the documenting of all data and code in a manner that the respective computation can be executed with the same results
...
## Motivation for Reproducibility { .incremental }
...
-   Made and by Jennifer Bryan et al. in 2018
...
Ulist <- runif(n, 0, 1)
Xlist <- lambda*((-log(1-Ulist))^(1/k))

par(mfrow = c(1,2))

hist(Xlist)
plot(pWeibull(ppoints(n), shape = k, scale = lambda),
     sort(Xlist),
...
![](restyle_false.png){width="80%" fig-align="center"}
...
![](restyle_true.png){width="80%" fig-align="center"}
